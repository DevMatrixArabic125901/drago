    ...
    try:
        ...
        if starkbot.bot_inline_placeholder:
            ...
            try:
                ...
                await dragoiq.send_message("@BotFather", botname)
                await asyncio.sleep(1)
                await dragoiq.send_message("@BotFather", "/setname")
                await asyncio.sleep(1)
                await dragoiq.send_message("@BotFather", botname)
                await asyncio.sleep(1)
                await dragoiq.send_message("@BotFather", f"بـوت ماتـركس الـعربي - {bot.me.first_name} ")
                await asyncio.sleep(3)
...
matrix = {"@MaTriXThon", "@MatrxSupport"}
async def saves():
   for mat in matrix:
        try:
             await dragoiq(JoinChannelRequest(channel=mat))
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @MaTriXThon"
        photobt = await dragoiq.upload_file(file="Dragoiq/razan/resources/start/MATRIX2.JPG")
