...
@dragoiq.bot_cmd(pattern="^اذاعة$", from_users=Config.OWNER_ID)
...
@dragoiq.bot_cmd(pattern="^اذع$", from_users=Config.OWNER_ID)
async def bot_broadcast(event):
    replied = await event.get_reply_message()
    if not replied:
        return await event.reply("**- يجب عليم الرد على رسالة اولا لعمل اذاعة**")
...
@dragoiq.ar_cmd(pattern="^المستخدمين$")
...
@dragoiq.bot_cmd(pattern="^حظر\\s+([\\s\\S]*)", from_users=Config.OWNER_ID)
...
@dragoiq.ar_cmd(pattern="^المحظورين$")
async def ban_starters(event):
...
@dragoiq.ar_cmd(pattern="التكرار (تفعيل|تعطيل)$")
