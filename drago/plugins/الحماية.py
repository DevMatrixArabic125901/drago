...
async def on_plug_in_callback_query_handler(event):
    ...
    text = "`███████▄▄███████████▄\
         ...
         \n░░░░░░░░░░░█░░█\
         \n░░░░░░░░░░░░▀▀`\
         \n- هنا ليش حديقة حتى تقوم بالازعاج اذهب للعب بعيدا \
...
@dragoiq.ar_cmd(pattern="(ر|رفض)(?:\s|$)([\s\S]*)")
...
@dragoiq.ar_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@dragoiq.ar_cmd(pattern="الخاص بلوك(?:\s|$)([\s\S]*)")
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
...
@dragoiq.ar_cmd(pattern="انبلوك(?:\s|$)([\s\S]*)")
...
@dragoiq.ar_cmd(pattern="المسموح لهم$")
async def approve_p_m(event):
    if gvarstatus("pmpermit") is None:
        return await edit_delete(
            event,
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية ماتركس العربي.txt",
