...
async def save_media(event):
    ...
    try:
        message = await dragoiq.get_messages(int(channel_id), ids=int(message_id))
        if not message:
        ...
        if message.media or message.document:
            ...
            await dragoiq.send_message(event.chat_id, file_ext)
            file_path = os.path.join(save_dir, f"media_{file_ext}")
            await dragoiq.download_media(message, file=file_path)

            await dragoiq.send_file('me', file=file_path, caption=message.text)
...
async def _(event):
    ...
    if not reply:
        return await edit_delete(
            event, "᥀︙ يجـب عليـك الرد عـلى الملصق لتحويـله الـى صورة"
        )
    output = await _cattools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
            output[0], "᥀︙ غـير قـادر على تحويل الملصق إلى صورة من هـذا الـرد"
        )
...
async def Hussein(event):
    ...
    try:
        channel_entity = await dragoiq.get_entity(channel_username)
        messages = await dragoiq.get_messages(channel_entity, limit=limit)
    except Exception as e:
    ...
    for message in messages:
        try:
            if message.media:
                ...
                await dragoiq.send_file("me", file=file_path)
                os.remove(file_path)
...
async def _(event):
    ...
    if not reply:
        return await edit_delete(
            event, "᥀︙ يجـب عليـك الرد عـلى الصـورة لتحويـلها الـى مـلصق"
        )
    output = await _cattools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
            output[0], "᥀︙ غـير قـادر على استـخراج الـملصق من هـذا الـرد"
        )
    meme_file = convert_tosticker(output[1])
    await event.client.send_file(
        event.chat_id, meme_file, reply_to=reply_to_id, force_document=False
...
async def _(event):
    "Converts the required media file to voice or mp3 file."
    if not event.reply_to_msg_id:
        await edit_or_reply(event, "**᥀︙ يـجب الـرد على اي مـلف اولا**")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
        await edit_or_reply(event, "**᥀︙ يـجب الـرد على اي مـلف اولا**")
...
async def _(event):
    ...
    mediatype = media_type(event)
    if mediatype and mediatype != "video":
        return await edit_delete(event, "᥀︙ يجـب عليك الـرد على فيديو اولا لتحـويله")
    args = event.pattern_match.group(1)
    if not args:
        args = 2.0
    else:
    ...
    result = await vid_to_gif(inputfile, outputfile, speed=args)
    if result is None:
        return await edit_delete(event, "**᥀︙ عـذرا لا يمكـنني تحويل هذا الى متـحركة**")
    jasme = await event.client.send_file(event.chat_id, result, reply_to=reply)
    await _catutils.unsavegif(event, jasme)
    await catevent.delete()
    for i in [inputfile, outputfile]:
