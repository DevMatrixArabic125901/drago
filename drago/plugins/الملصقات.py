...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"drago_{userid}_{pack}_anim"
    elif is_video:
        return f"drago_{userid}_{pack}_vid"
    return f"drago_{userid}_{pack}"
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            if message.media.document.mime_type == "video/webm":
                ...
                catevent = await edit_or_reply(args, "__جارِ التحميل عزيزي..__")
                sticker = await animator(message, args, catevent)
...
async def dragopkg(_):
    dra = await _.get_reply_message()
    if not dra:
    ...
    _id = dra.media.document.attributes[1].stickerset.id
    _hash = dra.media.document.attributes[1].stickerset.access_hash
    ...
    await edit_or_reply(
        _, f"**- تم اخذ الحزمة بنجاح ✓ \nالحزمة  → [اضغط هنا](https://t.me/addstickers/{drago_dra.set.short_name})**")

@dragoiq.on(admin_cmd(pattern="حزمه"))
async def dragopkg(_):
    dra = await _.get_reply_message()
    if not dra:
        return await edit_or_reply(_, "**- يجب عليك الرد على حزمة.**")
    _stickerset = None
    for attr in dra.media.document.attributes:
...
@dragoiq.on(admin_cmd(pattern="صنع ملصق(?:\s|$)([\s\S]*)"))    
async def kang(args):  
    photo = None
    emojibypass = False
    is_anim = False
