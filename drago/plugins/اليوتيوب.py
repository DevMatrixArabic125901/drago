...
from . import BOTLOG, BOTLOG_CHATID, dragoiq

BASE_YT_URL = "https://www.youtube.com/watch?v="
extractor = URLExtract()
LOGS = logging.getLogger(__name__)

plugin_category = "search"
...
async def ytdl_down(event, opts, url):
    ytdl_data = None
    try:
        await event.edit("**يتم جلب لك البيانات انتظر قليلاً**")
    ...
    for url in urls:
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(dragoiqevent, "__Unable to upload file__")
        await dragoiqevent.edit(
    ...
    for url in urls:
        ...
        try:
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(
                        d, t, dragoiqevent, c_time, "Upload :", file_name=ytdl_data["title"]
                    )
                ),
            )
            ul.close()
...
async def Ahmed_pin(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await dragoiq(unblock("bajardefacebookbot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(link)
            await conv.get_response()
...
async def Ahmed_gif(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await dragoiq(unblock("FullSavebot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(link)
            await conv.get_response()
...
async def Ahmed_mat(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await dragoiq(unblock("savetok_bot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(link)
            await conv.get_response()
