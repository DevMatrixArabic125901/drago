...
async def memes(event):
    ...
    ahmedinput = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    ...
    if gvarstatus("MATRIX_FONTS") is None:
        MATRIX_FONTS = "drago/helpers/styles/Terserah.ttf"
    else:
        MATRIX_FONTS = gvarstatus("MATRIX_FONTS")
    if max(len(top), len(bottom)) < 21:
        await ahmed_meme(MATRIX_FONTS, top, bottom, meme_file, meme)
    else:
        await ahmed_meeme(top, bottom, MATRIX_FONTS, meme_file, meme)
...
async def lang(event):
    ...
    if not input_str:
        ...
        return
    if input_str == "عربي":
        arr = f"drago/helpers/styles/zarz.ttf"
        addgvar("MATRIX_FONTS", arr)
    ...
    if input_str not in font_list:
        ...
        arg = f"drago/helpers/styles/{input_str}"
        addgvar("MATRIX_FONTS", arg)
...
async def memes(event):
    ...
    ahmedinput = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "**᭡︙قم بالـرد ع وسـائط مدعومـه ...**")
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
...
async def memes(event):
    ...
    ahmedinput = event.pattern_match.group(1)
    ahmedinput = int(ahmedinput) if ahmedinput else 50
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "**᭡︙قم بالـرد ع وسـائط مدعومـه ...**")
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    ...
    try:
        await crop(meme_file, outputfile, ahmedinput)
    except Exception as e:
...
async def memes(event):
    ...
    if not ahmedinput:
        ahmedinput = "50"
    if ";" in str(ahmedinput):
        ahmedinput, colr = ahmedinput.split(";", 1)
    else:
        colr = 0
    ahmedinput = int(ahmedinput)
    try:
        colr = int(colr)
    except Exception as e:
    ...
    try:
        await add_frame(meme_file, outputfile, ahmedinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
