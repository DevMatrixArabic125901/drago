...
async def fetch_info(replied_user, event):
    ...
    matrtba = (
"مطـور السـورس"
        if user_id == 6373798952 or user_id == 6162029418
        else ("العضـو")
    )
    matrtba = (
import os
...
async def who(event):
    mat = await edit_or_reply(event, "**⌔∮ جار التعرف على المستخدم انتظر قليلا**")
    ...
    try:
        ...
        return await edit_or_reply(
            mat, "**⌔∮ لم يتم العثور على معلومات لهذا المستخدم **"
        )
    ...
    try:
        ...
        await mat.edit(caption, parse_mode="html")
...
async def permalink(mention):
    ...
    await edit_or_reply(mention, f"⪼  [{tag}](tg://user?id={user.id})  𓆰. ")"مـالك الحساب"
        if user_id == (await event.client.get_me()).id
        and user_id != 6373798952
        and user_id != 6162029418
        else matrtba
